...
if [[ "${VERIFY_SKIP_INSTALL:-}" == "1" ]]; then
  ...
  run_check_verbose "install dependencies" "cd '$ROOT_DIR' && pip install -r requirements.txt"
fi

if [[ "${VERIFY_SKIP_TESTS:-}" == "1" ]]; then
  warn "skipping tests (VERIFY_SKIP_TESTS=1)"
else
  run_check_verbose "run unit tests" "cd '$ROOT_DIR' && python -m pytest test_main.py"
fi
